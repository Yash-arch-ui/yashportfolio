...
/**
 * MaskedHeading — a heading whose text is filled with an image or video.
 * Features parallax on pointer move, ambient drift, and rise/wipe/fade
 * reveal animations (ported from @react-bits/MaskedHeading, converted to
 * plain JSX for this project).
 *
 * The real text stays in the DOM for screen readers / SEO; the visual is a
 * clipped media layer over invisible measurement text.
 */
const MaskedHeading = ({
  ...
  // Kept in a ref so the rAF loop / sync always read the latest values without
  // re-creating callbacks. Runs before the other effects (declared first).
  useEffect(() => {
    ...
    >
      ...
      <span ref={revealRef} className="pointer-events-none absolute inset-0 block">
        <span
          ...
          style={{ clipPath: `url(#${clipId})`, backgroundColor: '#e2e8f0' }}
        >
